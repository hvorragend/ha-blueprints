...
### The shading-end waiting time is canceled by a *recovered* end condition, not only by a returned start condition (#696, CCA 2026.09.11)

**Problem.** The `shading_waitingtime_end` description promises that shading ends only if an
end condition is not fulfilled *for the entire waiting time*. The implementation only
*sampled* the end conditions: once when `t_shading_end_pending_*` armed the pending and once
when `t_shading_end_execution` fired (then again on every retry of the max-duration loop).
What happened in between was invisible. The #554 cancel branch ("Shading re-detected. Cancel
pending shading end") closed part of the gap, but it lives inside the *start* handler: it needs
a `t_shading_start_pending_*` edge **and** `shading_start_conditions_met`, i.e. brightness
above the *start* value (plus every other AND-group start condition). Brightness bouncing
between the end and the start value — the normal changeable-weather case — never canceled
anything, so on a day with broken clouds the shading ended as soon as one sample landed on a
cloud.

**Decision.** Every end-pending trigger has a mirrored `t_shading_end_cancel_N` trigger (same
numbering: 1 temp1, 2 temp2, 3 brightness, 4 weather, 5 azimuth, 6 forecast-temp sensor,
7 elevation, 8 custom sensor) that fires when the condition is *valid again*. They enter the
end handler through its `^(t_shading_end)` prefix, fall into the `else` of
`shading_end_conditions_met` (recovered ⇒ not met) and the branch "Shading end conditions
recovered. Cancel pending shading end" clears `pnd`/`ts.due`/`ts.arm` while `shd` stays 1
(no drive, no `man` change — Invariants 7 and 8). The next drop is a fresh `false → true`
edge of the end-pending trigger (its template went false at the recovery), so the waiting
time restarts from zero. The forecast and calendar load gates match the `t_shading_end`
prefix, so the `shading_end_conditions_met` re-check inside the branch sees fresh data.

**The cancel threshold is the nominal end value, not `end − hysteresis`.** The arm edge is
`< end − hysteresis` (unchanged); the cancel edge is `>= end` (temperatures: `>= threshold`,
forecast temperature: `>= forecast_temp`, sun: strictly inside the range, weather: state back in
the list, custom: `on`/`true`). With the default hysteresis of 0 both edges coincide and the
semantics are exact. With a hysteresis, `[end − h, end)` is a dead band in which neither edge
fires — a sensor jittering around the threshold does not toggle the wait on and off. Inside
the band the pending simply keeps running and the execution re-check samples as before; that
is the price of the dead band and is documented in the input description. Do not "fix" the
cancel to `>= end − hysteresis` for symmetry: that removes the only anti-flap protection of
the arm/cancel pair (the start/end distance protects start↔end, not arm↔cancel).

**Gate.** The global trigger gate admits `t_shading_end_cancel_[1-8]` only while the helper
carries `"pnd":"end"` (regex, like the other gate clauses). Orphan audit (recovery.md): a
suppressed or dropped cancel run costs nothing new — the pending stays armed and the
execution re-check falls back to the previous sampled behavior ("continue waiting" or the
stale-pending clear). The cancel triggers do latch (the condition stays valid), but the pending
they would cancel always ends through `t_shading_end_execution`, which is itself repaired by
`pending_is_stale` in the recovery gate.

**What it deliberately does not cover.** Conditions without a live sensor edge — the forecast
temperature/condition from a weather service (`forecast_temp_raw` without a dedicated sensor),
the time window, the user's additional end condition — are still only re-checked at execution
time. The retry loop (`shading_end_max_duration`) stays as it is for exactly those; with live
sensors it is now rarely reached because the cancel fires first.

**The #554 branch stays.** It still covers the start-handler entry (start conditions fully
back) and is the only cancel path when the recovered value never crosses the end value but
the start conditions are met through another route (independent temperature comparison).
Both branches write the same terminal pending state.

Pinned by `tests/test_shading_end_cancel_on_recovery.py`.
