...
- `recovered_state` — **mirrors `effective_state`, but on `new_base` and `live_force`**. A caught-up closing adds one branch-specific projection: when its tilted VENT leaf is refused by `auto_ventilate_condition`, `recovered_cascade_window` becomes `'cls'` for target selection so C-B falls through to C-E. Every other recovery keeps the real recovered window in the cascade; notably the shading-start ventilation floor has no live user-condition gate. The duplication is deliberate: `effective_state` reads `helper_json.bas`/`.frc`, which are exactly the stale values the recovery must correct, and HA templates cannot be parameterized. **Keep both in sync — every change to the `effective_state` cascade must be applied to `recovered_state` (Invariant 13).** `TestCascadeParity` renders both side by side; `TestRecoveredStateUsesRecoveredInputs` additionally feeds them *diverging* base/force, which parity by construction cannot see.
...
- `defer_to_shading_end` — the end mirror (#674, Bug Pattern AW): when the recovery itself just armed an **end** pending (the end conditions already hold) and the target would be `shd`, the drive is skipped too — driving into the shading position would be reverted by the shading end one waiting period later (the visible case: an instance take-over pulls a fully open cover down to the shading depth for five minutes). A live force-shade keeps its own authority (`live_force == 'non'` clause). The helper write (shd kept, pending armed) is unaffected.
...
- **Manual override: state transitions continue, only their effects are gated**
  ...
  `auto_override_reset_action`, even when the reconciliation is a no-op. A live
  `t_reset_timeout` / `t_reset_fixedtime` / `t_reset_position` sets
  `manual_reset_event`, is claimed by this reducer before the numbered reset
  branch, and enables the *state* catch-up regardless of `enable_recovery`: the
  release re-derives the scheduled base instead of trusting a possibly stale
  `bas`. Because this route consumes `recovered_pending` and `recovered_base`,
  the forecast and calendar load gates must include `manual_reset_event` too.
  Whether the release also *drives* is `auto_recover_after_manual_reset`
  (default off) via `manual_reset_recovery_hold`, independent of
  `enable_recovery` in both directions — see above.
...
| `t_shading_*_pending_*` (numeric/template) | **yes** (condition stays true) | shading never starts/ends that day. The sensor-based end triggers carry `for: shading_waitingtime_end` (#696), so a restart *during* that wait drops the edge just like a restart at the edge itself | the recovery gate's `recovered_pending` (re-evaluates and re-arms, the end pending with the full waiting time in the helper) | **opt-in** |
...
**Since CCA 2026.08.01 (#655/#656 audit) the flip gates are SHARED projections,
not mirrors.** The live branch entries and recovery both consume the action-level
`base_gates.{opening,closing}` projections. `once_ok` and `schedule_ok` decide
the transition; `override_ok` decides only its movement, so the desired base
state stays current under manual control (Invariant 15). The semantic
...
expired-override composition, the night clause, the any-`man` drive gate,
