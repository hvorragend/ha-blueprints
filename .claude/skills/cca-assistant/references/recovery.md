...
- **Manual override: direction- and outcome-specific for caught-up transitions, conservative for pure re-positioning** (CCA 2026.08.01, #656 audit). A caught-up flip already passed `base_gates.*.override_ok`, but that authorizes only a target its live subbranch could have driven: opening accepts `opn`/`lock`, or `shd` when an active shading owns O-C; closing accepts `cls`/`vnt`/`lock`. `caught_up_opening_hold` / `caught_up_closing_hold` reject unrelated cascade targets before the manual exemption can matter. A recovery run *without* a base transition has no live event to replay, so `manual_holds_reposition` blocks that drive on any `man == 1` — a named recovery-only policy, not part of the parity model (recovery-parity.md R2). All three live in the drive path, **not** in the branch conditions, so the helper hygiene always runs; only a live-authorized lockout can overrule the manual flag, per Invariant 6. When `override_expired` clears `man`, the branch also runs the user's `auto_override_reset_action`, exactly as BRANCH 10 does — a reset caught up by the recovery must not silently skip the notification/scene the user wired to it.
