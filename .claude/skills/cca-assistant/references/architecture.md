...
**BASE=OPN beats VENT only when an opening automation actually exists** (`is_opening_scheduled`). `bas` initializes to `'opn'` and is only ever switched to `'cls'` by the close handler — so in a shading-only setup with no opening automation, `bas` stays `'opn'` permanently. Without the gate the VENT floor could never apply there (Issue #553, Bug Pattern Z). When no opening automation exists, a tilted window therefore still produces `vnt`. Since #673 (Bug Pattern AV) the opening and closing branches also run as pure **state syncs** with their feature unchecked — the schedule flips to `bas='opn'` / `bas='cls'` persist while the drives stay feature-gated (`is_up_enabled` in the normal-opening `will_drive`; `is_down_enabled` in the normal-closing and tilted-ventilation `will_drive`); the sync writers are deliberately **not** `is_opening_scheduled` / `is_closing_scheduled` sources, because they never drive. Every automatic drive toward the close position outside the closing branch gates on `closing_target_owned` (scheduled closing exists, or the live resident privacy owns the target) — see Bug Pattern AV. The user's `auto_up_condition` is the same class of effect gate on the open target (#698, Bug Pattern AY): it cannot join `is_opening_scheduled` (a `!input` condition is not expressible in `trigger_variables`), so it is evaluated at drive time wherever the open position is the destination — the normal-opening `will_drive`, `recovery_up_condition_hold`, the shading-end move, the window-closed return and `target_condition_gate` — and never gates the base transition.
