...
**BASE=OPN beats VENT only when an opening automation actually exists** (`is_opening_scheduled`). `bas` initializes to `'opn'` and is only ever switched to `'cls'` by the close handler — so in a shading-only setup with no opening automation, `bas` stays `'opn'` permanently. Without the gate the VENT floor could never apply there (Issue #553, Bug Pattern Z). When no opening automation exists, a tilted window therefore still produces `vnt`. Since #673 (Bug Pattern AU) the opening and closing branches also run as pure **state syncs** with their feature unchecked — the schedule flips to `bas='opn'` / `bas='cls'` persist while the drives stay feature-gated (`is_up_enabled` in the normal-opening `will_drive`; `is_down_enabled` in the normal-closing and tilted-ventilation `will_drive`); the sync writers are deliberately **not** `is_opening_scheduled` sources, because they never drive.
