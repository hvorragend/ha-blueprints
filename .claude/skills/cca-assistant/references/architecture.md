...
**BASE=OPN beats VENT only when an opening automation actually exists** (`is_opening_scheduled`). `bas` initializes to `'opn'` and is only ever switched to `'cls'` by the close handler — so in a shading-only setup with no opening automation, `bas` stays `'opn'` permanently. Without the gate the VENT floor could never apply there (Issue #553, Bug Pattern Z). When no opening automation exists, a tilted window therefore still produces `vnt`. Since #673 (Bug Pattern AU) the opening branch also runs as a pure **state sync** with Morning Opening unchecked — the schedule flip to `bas='opn'` persists while the opening drive stays feature-gated (`is_up_enabled` in the normal-opening `will_drive`); this sync writer is deliberately **not** an `is_opening_scheduled` source, because it never drives.
