...
- 🐛 **Fix:** With a window **tilted** (tilt contact alive and reporting) while its *opened* contact sensor was **unreachable** (typical for battery contacts, which some hubs mark unavailable after hours without an event), the **evening closing drove the cover fully closed over the tilted window** instead of stopping at the ventilation position — and the ventilation handling at the end of a sun shading skipped the ventilation position the same way. The `2026.07.20` availability fix deliberately lets these runs through, because the live tilt contact is the sensor that actually guards the ventilation position — but the ventilation handling itself still treated the unreachable opened contact as "possibly fully open" and stepped aside, and of the three possible outcomes (keep open, ventilation position, full close) the cover got the least safe one. The ventilation handling now applies the same rule as the availability protection: an unreachable opened contact only counts as "possibly open" while the window was last known fully **open** — for a tilted or closed window, "no reading" agrees with the last known state and the tilted window keeps its ventilation position ([#631](https://github.com/hvorragend/ha-blueprints/issues/631), follow-up to [#622](https://github.com/hvorragend/ha-blueprints/issues/622))
