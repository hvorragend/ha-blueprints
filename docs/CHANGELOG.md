...
- 🐛 **Fix:** An **🔼 Additional Condition For Opening The Cover** no longer freezes CCA's internal day state while it is false ([#698](https://github.com/hvorragend/ha-blueprints/issues/698)). The condition was checked at the very entrance of the opening handler, so with the condition false (e.g. the vacation-mode pattern from the FAQ, where the covers only open automatically while a vacation switch is on) not even the pure bookkeeping ran: the stored base state stayed on "closed" around the clock — the same failure shape as [#673](https://github.com/hvorragend/ha-blueprints/issues/673), reached through the condition instead of the Morning Opening checkbox. A cover opened **by hand** on such a day was therefore closed again by the next automatic event that consulted the stored state: the end of a sun shading, a 🖐️ Manual Override reset with return-to-target enabled, switching off a force function, or a restart catch-up. The opening time now advances the day state even while the condition is false — **without moving the cover** — and the condition instead withholds exactly the movements **toward the open position**: at the opening time itself and at every later return to the open position (end of sun shading, window closed after ventilation, restart catch-up, Manual Override reset, force disable). A cover you open by hand is now simply treated as open for the rest of the day. This is the behavior the FAQ already described for the per-action conditions ("only that one movement is suppressed, the background state tracking stays intact")
- 🔧 **Improvement:** Consequence for "covers stay closed during vacation" setups that also have sun shading configured: because the day state now reads "open" while the opening condition refuses, a sun shading that starts on a still-closed cover raises it to the shading position (previously the shading was only noted for later), and at the shading end the cover stays there instead of opening. If a cover must not move at all during vacation, add the same vacation condition to **🥵 Additional Condition When Activating Sun Shading**, or use ☝️ Force Close — see the vacation entry in the [FAQ](https://hvorragend.github.io/ha-blueprints/FAQ)
- 🔧 **Improvement:** The description of the 🔼 Additional Condition For Opening The Cover now says what it suppresses and what it leaves alone. The **🔻 Additional Condition For Closing The Cover** deliberately keeps its previous behavior (while it is false, the closing handler does not run at all, including its state update) — its automatic consumers are built on a different ownership rule and a mirror needs its own, separate change
...
- 🐛 **Fix:** A restart catch-up or instance take-over no longer drives the cover **into** a sun shading whose end conditions are already met ([#674](https://github.com/hvorragend/ha-blueprints/issues/674)). When the recovery found the status helper still marked as shaded but the sun had already left the shading window, it armed the shading-end waiting period **and** simultaneously moved the cover to the shading position — e.g. an instance activated via its "Only start if this entity is on" switch in the late afternoon pulled a fully open cover down to the shading position, only for the shading end to move it back one waiting period later. The recovery now leaves the cover where it is in that situation and lets the armed shading end perform the one real movement — mirroring how a restart catch-up already defers a pending shading **start** to its execution instead of opening first. A live ☝️ Force sun shading keeps its own authority and is still positioned immediately, and a helper marked shaded whose end conditions do **not** hold is still caught up to the shading position exactly as before

# CCA 2026.08.23

- ✨ **Feature:** New setting **"Manual movement inside the schedule window counts as the scheduled event"** (`manual_schedule_adoption`, default: nothing selected) for everyone who prefers to open or close the cover by hand ([#671](https://github.com/hvorragend/ha-blueprints/issues/671)). When enabled for a direction, a manual movement to the open (or closed) position **while the matching schedule window is running** — between the early and late opening/closing times, or during the calendar event — advances CCA's internal day state exactly as the scheduled event would: later automatic events (the end of sun shading, a Manual Override reset with return-to-target enabled, a restart catch-up) no longer treat the scheduled opening/closing as still outstanding, and the once-per-day guards count the manual movement as that day's event. Only the internal state advances — the movement is still recorded as a normal 🖐️ Manual Override, the cover is never moved by this setting, and outside the window a manual movement stays a plain Manual Override as before
- 🐛 **Fix:** Tilting the window while sun shading is active now moves a **tilt/venetian cover** to the 💨 Ventilation Position even when the cover does not rest *exactly* at that position ([#667](https://github.com/hvorragend/ha-blueprints/issues/667)). In venetian setups the closed, shading and ventilation cover positions typically share one value (e.g. 0%) and only the slat angle differs — but many motors report the resting position a point or two off the commanded value, especially after tilt movements. The "window tilted" check required exact position equality, so the tilt event was swallowed entirely: no slat movement and no status update. The check now accepts a cover within the **Position Tolerance** whose slats are not yet at the ventilation angle; the same correction applies to the "ventilation after sun shading ends" path. Covers already fully in the ventilation position (including slat angle) keep the previous no-movement behavior, and covers clearly above the ventilation position still require the option *"💨 Lower cover to ventilation position when window tilts and cover is above"* — nothing changes for those cases
...
- 🐛 **Fix:** The **mirror of the #673 configuration** — **🔼 Morning Opening configured but 🔻 Evening Closing unchecked**, for everyone who closes by hand in the evening — no longer treats the whole night as day ([#673](https://github.com/hvorragend/ha-blueprints/issues/673), [#677](https://github.com/hvorragend/ha-blueprints/issues/677)). Internally the stored base state stayed on "open" around the clock, so after a manual evening close the next reconciliation — e.g. the 🖐️ Manual Override expiry a few hours later — **reopened the cover in the middle of the night**, and a tilted window on a manually **closed** cover did not get its 💨 Ventilation Position at all. The closing time now updates the stored night state even when Evening Closing is unchecked — **without moving the cover**: the flip respects the same time window, brightness/sun checks, additional closing condition and once-per-day rule as a real closing, and the closing movement stays strictly disabled
- 🐛 **Fix:** Because a stored "closed" state could previously only exist when a closing automation was configured, several automatic movements silently assumed exactly that. They now check explicitly **who owns the close target** before moving: without a closing automation (and without an active 🧍 resident privacy close, which keeps its authority) the end of a night ventilation no longer closes a deliberately open cover, tilting a window no longer pulls it down to the ventilation position, a sun shading that ends in the evening no longer performs the closing, and switching off a force function or force pause — as well as the restart catch-up and the opted-in Manual Override reset — leaves the cover where it is. A cover that was closed **by hand** still returns from its ventilation position to closed when the window closes, and a live force-close keeps its own authority everywhere. For every configuration with Evening Closing enabled nothing changes at all
...
- 🐛 **Fix:** When the new setting above is enabled, a reset no longer applies a stale opening/closing state from the status helper. This could otherwise close a cover during the daytime after it had been opened manually, when the scheduled opening transition had previously been missed or refused. The reset re-derives the current target from the live schedule and the same opening/closing, calendar, weather, window and Manual Override gates used by restart recovery, independently of the optional restart/outage catch-up setting ([#655](https://github.com/hvorragend/ha-blueprints/issues/655), follow-up to [#657](https://github.com/hvorragend/ha-blueprints/pull/657))
- 🔧 **Improvement:** The description of **"Automatic movements blocked during Manual Override"** now says explicitly that leaving all four choices unselected means Manual Override protects nothing — the next relevant automatic movement simply overrides it, ending the override in the process, without needing a separate reset
- 🔧 **Improvement:** In the Force Features section, **"Return to Target State After Force Disable"** now sits after the four Force entities it actually applies to, and both its description and **"Force Pause"**'s now cross-reference each other, so it is clear the setting does not apply to Force Pause (which always returns immediately, unconditionally)
- 🔧 **Improvement:** The description of **"Return to Target State After Manual Override Reset"** now notes a residual edge case: if this instance's closing schedule is disabled after having closed the cover at least once, an enabled reset can still drive it closed on stale grounds — unlike opening, there is no guard for this, so keep the matching closing schedule enabled if you rely on this option ([#673](https://github.com/hvorragend/ha-blueprints/issues/673))
