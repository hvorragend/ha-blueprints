...
- 🐛 **Fix:** The **mirror configuration** — **🔼 Morning Opening configured but 🔻 Evening Closing unchecked**, for everyone who closes by hand in the evening — had the same flaw in the other direction: the stored base state stayed on "open" all night, so after a manual evening close the next reconciliation (e.g. the 🖐️ Manual Override expiry a few hours later) **reopened the cover in the middle of the night**. The closing time now updates the stored night state the same way — **without moving the cover**: the closing movement (including the "window tilted → ventilation position instead of closing" movement, which belongs to the evening closing) stays strictly disabled, a tilted window at night still gets its ventilation position through the normal window handling, and the full-window lockout protection remains untouched as the safety exception
