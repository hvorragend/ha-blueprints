...
- 🐛 **Fix:** When a resident left (presence `on → off`) while a window was tilted, the cover stayed in the ventilation position instead of opening fully. With the resident gone and the schedule set to "open" (`bas=opn`), ventilation is only a *floor* — the cover should open completely because no ventilation is needed at that point. The "Resident leaving: target VENTILATION (window tilted)" branch only checked the tilted sensor and drove to the ventilation position regardless of the effective target. It now also requires `effective_state == 'vnt'`, so when the base state is open the branch is skipped and the "target OPEN" branch fully opens the cover; at night (`bas=cls`) ventilation remains the correct floor. This matches the priority cascade (BASE=OPN before VENT) already used by the contact handler
