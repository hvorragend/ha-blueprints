...
- 🐛 **Fix:** 🖐️ **Manual override and force blocking no longer freeze CCA's internal state machine** ([#655](https://github.com/hvorragend/ha-blueprints/issues/655)). Previously a manual movement erased active or pending ☀️ sun shading, while several opening, closing, shading and ventilation paths skipped their complete state transition whenever manual or a conflicting force prevented the movement. The helper then remembered the user's temporary physical position instead of CCA's current target; when control returned, no new temperature, brightness or sun trigger edge remained and the cover stayed wrong. CCA now continues its schedules, shading timers, window state and resident state normally and suppresses only the actual drive command. If shading becomes due during an override, `shd` becomes active without moving; if it ends again, that is recorded too. Position, timeout and fixed-time reset immediately reconcile the resulting current target, while full-window lockout remains the safety exception and explicit force targets retain priority. The same state/effect separation applies while Force Pause or another force target prevents a background movement
- 🐛 **Fix:** The **restart/outage catch-up** now derives a missed opening or closing from the **same shared state and drive gates as the regular flow**, instead of maintaining its own approximation. This includes the time window and brightness/sun checks reported in [#656](https://github.com/hvorragend/ha-blueprints/issues/656), the user's additional opening/closing conditions, 🖐️ **manual override**, and the **once-a-day options** (*"Prevent opening/closing several times a day"*). An active manual override no longer refuses the transition itself: both live and recovery persist the current base state while suppressing only its configured movement, so releasing manual control can immediately reconcile the up-to-date target. A cover that already made its one scheduled movement of the day is still not moved a second time merely because a restart or recovery re-derived the schedule
- 🐛 **Fix:** A **closing caught up by the restart/outage recovery** now ends an active ☀️ **sun shading**, exactly like the regular evening closing does. Previously the stored shading survived the catch-up, outranked the closed state (as it is designed to do during the day), and the recovery drove the cover to the **shading position for the night** instead of closing it — the closing it was supposedly catching up. An armed shading waiting period is dropped along with it, exactly as the regular closing drops it
- 🐛 **Fix:** A closing caught up by the recovery now respects the situations in which the regular closing deliberately does **not move the cover**: the **"Prevent lowering when closing if shaded"** and **"Prevent higher position on closing"** options and the **tilted-window lockout option** (*"Prevent when window is tilted"* for closing). Where the regular closing would only have recorded the closed state and left the cover where it stands, the catch-up now does the same. Recovery runs that merely re-establish an unchanged state (e.g. after a window was closed during the outage) are unaffected — those behave like the regular return movements, which never used these options
- 🐛 **Fix:** Recovery now preserves the **branch-specific window, resident and force decisions** of the live transition instead of treating a passed opening/closing gate as permission to execute any target selected by the priority cascade. A refused user ventilation condition therefore blocks the same lockout movement as the live window-contact branch, while a tilted window may still fall through to a full closing where the live closing branch does; it does **not** globally remove the ventilation floor from unrelated flows such as active sun shading. Likewise, a resident-blocked caught-up opening cannot turn into a closing movement, and a caught-up closing cannot execute a conflicting force-open target. When a caught-up opening legitimately uses the open-window lockout target, it retains the regular opening actions rather than running the ventilation actions
- 🔧 **Improvement:** The parity guarantee is now explicit and regression-tested: reusable opening/closing predicates and user condition nodes are shared between the regular flow and recovery, while the few inherently branch-specific outcomes are represented by narrowly scoped recovery holds instead of being generalized into cascade rules. A paired test suite runs the same scenarios through both paths — including the exact situation from [#656](https://github.com/hvorragend/ha-blueprints/issues/656), manual and once-per-day gates, windows and ventilation conditions, resident/force overlays, shading and the no-movement closing options — and compares the transition, movement, target, action set and helper mutations. The deliberately recovery-specific reconciliation rules (for example repairing a manual override whose reset moment fell into the outage) are documented separately and pinned as explicit deviations rather than being described as identical live behavior
