...
- 🐛 **Fix:** **Time Control could not be disabled through the UI** for installations created after the options consolidation (~2026.05). Unchecking the *"⏲️ Time Control"* checkbox in *Automation Options* had no effect: internally, disabling additionally required the legacy selector value `time_control: time_control_disabled` — but that value was no longer offered in the *"Time Control Type"* dropdown, so the disable path was unreachable. Brightness-/sun-only setups were therefore still constrained by the default Early/Late time windows (e.g. a sun-elevation closing kept being gated by the 16:00–22:00 window). The *"Time Control Type"* dropdown now offers **🚫 Disabled** again and is the **single authoritative** way to turn the time windows off — selecting it disables time control immediately. **Warning:** without time windows there is no guaranteed *Late* opening/closing safety net. Configurations that still store the pre-consolidation value `time_control: time_control_disabled` keep meaning "time windows disabled", exactly as originally configured ([#544](https://github.com/hvorragend/ha-blueprints/issues/544))
- ⚠️ **Deprecated — backward compatibility removed:** The *"⏲️ Time Control"* checkbox in *Automation Options* is **dead**. It has **no function whatsoever** — checked or unchecked makes no difference, and the internal backward-compatibility logic that still evaluated it has been **removed without replacement**. This checkbox never worked as a standalone disable switch (that is exactly the bug fixed above), so no working configuration can rely on it. The **only** setting that decides whether the Early/Late time windows are active is *"Time Control Type"* (🚫 Disabled = off, everything else = on). The checkbox entry currently remains visible in the list purely so that existing stored configurations stay valid; it is scheduled for **removal in a future release**. No action is required — but do not expect the checkbox to do anything ([#544](https://github.com/hvorragend/ha-blueprints/issues/544))
