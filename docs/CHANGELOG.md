...
- 🐛 **Fix:** The sun shading no longer ends after a brief cloud although the sun was shining again seconds earlier ([#696](https://github.com/hvorragend/ha-blueprints/issues/696)). The *"Sun Shading - End Waiting Time"* promises that shading ends only if an end condition is not fulfilled **for the entire waiting time** — but the sensor-based end conditions were only *sampled*: once at the first drop (arming the waiting time) and once when it expired (plus every further waiting time in the retry loop). What happened in between was invisible, so on a day with changeable weather the shading ended as soon as one of those checks happened to land on a cloud. Only a full return of the **start** conditions (e.g. brightness above the *start* value) canceled the waiting time ([#554](https://github.com/hvorragend/ha-blueprints/issues/554)). The sensor-based end triggers (brightness, temperature 1/2, forecast temperature sensor, weather condition, custom condition sensor) now measure the waiting time **themselves**: the value has to stay below/outside its end threshold for the whole waiting time without interruption — if it recovers in between, the wait starts over from zero — and only then is the shading end executed (the re-check of all end conditions at execution time stays). The sun-position end (azimuth/elevation leaving the range) keeps its waiting period as before, including the *"End Sun Shading - Immediately When Out Of Range"* option; a waiting period restored after a restart still runs in the status helper. Note: a Home Assistant restart or a save of the automation during a running sensor waiting time discards that wait — the shading then ends with the next drop of the value (or with the sun-position end / the closing time)
