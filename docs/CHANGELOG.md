...
- 🐛 **Fix:** The **restart/outage catch-up** now decides an opening or closing with **exactly the same checks as the regular flow** — it literally reads the same internal check the regular opening/closing branch reads, instead of keeping its own approximation of it. This closes the remaining gaps of the audit that produced the `2026.07.30` brightness/sun fix ([#656](https://github.com/hvorragend/ha-blueprints/issues/656)): a 🖐️ **manual override** with *"Ignore/override next automatic opening/closing after manual position changes"* configured now refuses the caught-up movement exactly like it refuses the regular one (an override whose reset moment fell into the outage is lifted first, and the catch-up then proceeds against the repaired state), and the **once-a-day options** (*"Prevent opening/closing several times a day"*) are honored too — a cover that had already made its one scheduled movement of the day is no longer moved a second time just because a restart or an outage recovery re-derived the schedule
...
- 🔧 **Improvement:** The guarantee behind all three fixes is structural, not a one-time alignment: the opening/closing decision checks (manual override, once-a-day, time window, brightness/sun, the no-movement closing rules) now exist **once**, as a shared set of named checks that both the regular flow and the restart/outage catch-up read. An internal test suite runs the same scenarios through both paths — including the exact situation from [#656](https://github.com/hvorragend/ha-blueprints/issues/656) — and fails if they ever disagree again, and structural tests reject any future change that re-introduces a private copy of one of these checks on either side. The few places where the catch-up deliberately decides differently (e.g. it may complete a movement whose manual override expired *during* the outage) are documented and pinned by their own tests
