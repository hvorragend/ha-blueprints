...
- 🐛 **Fix:** The sun shading no longer ends after a brief cloud although the sun was shining again seconds earlier ([#696](https://github.com/hvorragend/ha-blueprints/issues/696)). The *"Sun Shading - End Waiting Time"* promises that shading ends only if an end condition is not fulfilled **for the entire waiting time** — but the end conditions were only *sampled*: once at the first drop (arming the waiting time) and once when it expired (plus every further waiting time in the retry loop). What happened in between was invisible, so on a day with changeable weather the shading ended as soon as one of those checks happened to land on a cloud, even if the sun had been back out the whole time before. Only a full return of the **start** conditions (e.g. brightness above the *start* value) canceled the waiting time ([#554](https://github.com/hvorragend/ha-blueprints/issues/554)). Every end condition now has a matching "recovered" trigger: when the brightness rises back to the *end* value or above, the temperature climbs back to its threshold, the sun re-enters the azimuth/elevation range, the weather condition matches again or the custom condition sensor turns back on while the end waiting time is running, the waiting time is **canceled** and the cover stays shaded — it starts over from zero the next time the condition fails. The **Hysteresis** keeps its role as the dead band: a value that fell below *end − hysteresis* arms the waiting time and has to climb back to the *end value itself* to cancel it, so a sensor jittering around the threshold does not toggle the wait on and off. Conditions without a live sensor (forecast temperature from a weather service, a time window) are still re-checked at the end of the waiting time as before
