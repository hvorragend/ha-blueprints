...
- ✨ **Feature:** **Starting the automation by hand now re-syncs the cover** instead of doing nothing. A run started manually — the **"Run"** action in the automation UI, or `automation.trigger` from a script — carries no trigger information, so none of CCA's internal branches could ever process it: the run ended silently with *"No operational branch matched"*, even when everything agreed the cover was in the wrong place (the reported case: all sun-shading conditions met, cover stuck open after the status helper had been corrected by hand — and because the shading triggers had already fired earlier, no natural trigger would come to the rescue that day either). A manual start is now treated as an explicit *"re-evaluate and fix it"* request: CCA re-reads the schedule, window contacts, presence, force switches and shading conditions from scratch, tidies the status helper and brings the cover to the state they demand — the same catch-up a restart recovery performs. Like a 🎚️ hand-over, it runs **regardless of the 🔄 Recovery setting**: the opt-in protects against restarts moving the cover unasked, and a manual start is the opposite of unasked. Manual overrides, the window lockout and a ⏸️ Force Pause are respected exactly as everywhere else, and a warranted sun shading arms its usual waiting period rather than shading instantly. This makes a manual run the supported way to re-sync CCA after external changes (status helper rewritten by hand, cover driven by another automation) ([#639](https://github.com/hvorragend/ha-blueprints/issues/639))
- 🐛 **Fix:** A manual run started while the **Cover Status Helper was still loading** (`unavailable`, e.g. right after a restart) could **overwrite the stored status with default values** — the UI "Run" action skips the availability protection that guards normal runs against exactly this. The status repair steps now check for it themselves, and a manual run whose cover, status helper or window contacts are not usable safely does nothing at all (it can simply be repeated once they are back)
