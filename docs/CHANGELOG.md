...
- ✨ **Feature:** CCA now **detects a configuration that lost its time control** to the 2026.07.12 breaking change ([#544](https://github.com/hvorragend/ha-blueprints/issues/544)) **by itself and raises a notification** — instead of silently opening the covers around sunrise. Configurations created before the options consolidation (~2026.05) store neither the **⏲️ Time Control** checkbox nor the old *Disabled* selector value, so the update switched their time windows off without a trace: with a hybrid setup (time + brightness/sun) the covers suddenly moved as soon as a sensor threshold was crossed, with pure time control they stopped moving at all (reported: the whole house opening at 5:30 am, [#595](https://github.com/hvorragend/ha-blueprints/issues/595)). An affected automation — time control off, but unused time-window settings (customized times, a workday sensor, or a calendar) still configured — now raises a **persistent notification** on the first run after a restart, a reload or a save (which includes the run right after updating the blueprint), naming the automation and both ways out: re-check **⏲️ Time Control** to restore the windows, or reset the unused time settings if the uncheck was deliberate. A warning also lands in the system log. Unlike the **✔️ Check Configuration** report above, this needs no manual run — it speaks up on its own. The detection is deliberately heuristic — a deliberately disabled time control on a clean configuration (default times, no workday sensor, no calendar) is never flagged, and a configuration that had explicitly chosen *Disabled* before the consolidation stays silent, matching its original intent. The blueprint cannot repair the stored configuration itself (Home Assistant offers no migration mechanism for blueprint inputs), so a loud, dismissible notification is the strongest safe response
