...
- 🐛 **Fix:** The **restart/outage catch-up** now derives a missed opening or closing from the **same shared entry gates as the regular flow**, instead of maintaining its own approximation. This includes the time window and brightness/sun checks reported in [#656](https://github.com/hvorragend/ha-blueprints/issues/656), the user's additional opening/closing conditions, 🖐️ **manual override**, and the **once-a-day options** (*"Prevent opening/closing several times a day"*). A configured *"Ignore/override next automatic opening/closing after manual position changes"* therefore refuses the caught-up transition just as it refuses the corresponding live transition (an override whose reset moment fell into the outage is repaired first, after which recovery reconciles the current state), and a cover that already made its one scheduled movement of the day is not moved a second time merely because a restart or recovery re-derived the schedule
