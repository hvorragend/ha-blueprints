...
- 🔧 **Improvement:** With *dynamic* sun-shading tilt (tilt angle following the sun elevation in stages), the *"is the cover in the shading position?"* check no longer goes stale the moment the sun moves to a new tilt stage. The blueprint now remembers the **last applied shading tilt** in the status helper and compares against it, instead of against the constantly-recomputed target. This keeps the shading state stable between stage changes and prevents redundant re-drives and false manual-override detection on venetian/tilt covers (follow-up to [#538](https://github.com/hvorragend/ha-blueprints/issues/538), [#558](https://github.com/hvorragend/ha-blueprints/issues/558)). The status helper is automatically migrated to the new format (schema v7) on the first run — no manual action required
