...
- 🐛 **Fix:** **Time Control could not be disabled through the UI** for installations created after the options consolidation (~2026.05). Unchecking the *"⏲️ Time Control"* checkbox in *Automation Options* had no effect: internally, disabling additionally required the legacy selector value `time_control: time_control_disabled` — but that value was no longer offered in the *"Time Control Type"* dropdown, so the disable path was unreachable. Brightness-/sun-only setups were therefore still constrained by the default Early/Late time windows (e.g. a sun-elevation closing kept being gated by the 16:00–22:00 window). The *"Time Control Type"* dropdown now offers **🚫 Disabled** again and is the **single authoritative** way to turn the time windows off — selecting it disables time control immediately, with no checkbox involved. **Warning:** without time windows there is no guaranteed *Late* opening/closing safety net. The *"⏲️ Time Control"* checkbox in *Automation Options* is now **deprecated and has no function** (it is kept only so existing configurations remain valid; checked or unchecked makes no difference). Existing installations are unaffected: configurations that still store the pre-consolidation value `time_control: time_control_disabled` keep meaning "time windows disabled", exactly as originally configured, and all other configurations keep their time windows active regardless of the checkbox ([#544](https://github.com/hvorragend/ha-blueprints/issues/544))
